...
router.post(
  ...
  [
    ...
    body('content', 'Description must be at least 5 characters').isLength({
      min: 5,
    }),
  ],
  async (req, res) => {
...
router.post(
  ...
  fetchuser,
  [
    body('text', 'Enter a valid comment text').isLength({ min: 1 }),
  ],
  async (req, res) => {
    try {
      const { text } = req.body;
      ...
      const newComment = new Comment({ text, author: req.user.id, parentPost: postId });
